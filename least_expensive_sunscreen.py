...
for each_element in spf_50:
        ...
        if(price_spf50<min_value):
            min_value=price_spf50
...
for each_element in spf_30:
        ...
        if(price_spf30<min_value):
            min_value_two=price_spf30

#find and click the Add button corresponding to the minimum price
driver.find_element_by_xpath("//p[contains(text(),'%d')]/following-sibling::button"%min_value_two).click()      
print("the sunscreen added to the cart is %s and its price is %d"%(driver.find_element_by_xpath("//p[contains(text(),'%d')]/preceding-sibling::p"%min_value_two).text,min_value_two))